...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function fillModal(user) {
      ...
      /*  document.getElementById('modalContactNo').textContent = user.contactNo || '-';*/
        ...
        // Header
        let headerRow = '<tr><th>Details</th>';
        ...
        donations.forEach(d => {
            ...
            rows.txn.push(`<td>${d.receiptType === 'o' || d.receiptType === 'O' ? 'Old Receipt' :
                d.receiptType === 'n' || d.receiptType === 'N' ? 'New Receipt' :
                    '-'
                }</td>`);
            ...
            rows.print.push(
                `<td><button style="font-size:11px; background-color:green;color:white;"
                     ...
                     data-donation-id="${d.receiptNo}">Print
                     <i class="fas fa-print" style="cursor:pointer;color:white;"></i>
                 </button></td>`
            );
        });
    ...
    function generateReceipt(user) {
        (user.donations || []).forEach(donation => {
            ...
            doc.text("ಶ್ರೀ ಪತ್ರೇಶ್ವರ ದೇವಸ್ಥಾನದ ಸೇವಾ ಟ್ರಸ್ಟ್ (ರಿ)", 105, 50, { align: "center" });
